...
def menu_screen(transaction, reqres):
    ...
    while key != getch.KEY_x:
        logger(''.join([Fore.CYAN,
                        "Type 'f' to view the screen's fields or 'p' to view the un-markedup screen, 'e' to view the screen as an emulator would, 'r' to switch between the Request/Response, or 's' to export a copy to a text file (.brp/.emu). Type 'x' to go back.",
                        Fore.RESET]), kind='info')
        key = getch()
        if key == getch.KEY_f or key == getch.KEY_F:
            print(Fore.BLUE, "View Fields", Fore.RESET)
            print(Fore.BLUE, "===========", Fore.RESET, "\n")
            pprint(screen.fields)
            logger(''.join([Fore.RED, "Dropping into shell, check the", Fore.BLUE, " screen ", Fore.RED,
                            "object. Type quit() to return here.", Fore.RESET, "\n\n"]), kind='info')
            embed()
            ...
            filename = transaction.host + '_' + str(transaction.timestamp.date()) + '_' + str(
...
def menu_trans(history, num):
    ...
    while key != getch.KEY_x:
        ...
        logger(''.join([Fore.CYAN,
                        "Choose '1' to view the Request, and '2' to view the Response. Type 'j' to move to the next transaction, and 'k' to the previous. Type 'x' to go back.",
                        Fore.RESET]), kind='info')
...
def menu_history(history):
    ...
    while choice.lower() != 'x':
        ...
        logger(''.join(
            [Fore.CYAN, 'Choose a transaction to view with the appropriate numeric key. Hit x, then enter to go back.',
             Fore.RESET]), kind='info')
...
def menu_find(history):
    ...
    while choice.lower() != 'x':
        logger(''.join(
            [Fore.CYAN, 'Type the text you would like to search for and hit enter. Hit x, then enter to go back.',
             Fore.RESET]), kind='info')
